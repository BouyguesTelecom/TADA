...
function getPathFromRegex(regexp) {
    return regexp.toString().replace('/^', '').replace('?(?=\\/|$)/i', '').replace(/\\\//g, '/').replace('(?:/(?=$))', '');
}
...
const expressListRoutes = (app, opts) => {
            ...
            if (stack.route) {
                ...
                for (const route of stack.route.stack) {
                    ...
                    if (!routeLogged[method] && method) {
                        ...
                        const stackPath = path.normalize([options.prefix, stack.routerPath, stack.route.path, route.path].filter((s) => !!s).join('')).trim();
